...
exports.trackCallUsage = exports.validateRoomAccess = exports.generateLiveKitToken = void 0;
...
exports.generateLiveKitToken = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to generate tokens');
        }
        ...
        if (!roomName.startsWith('privacycall_')) {
            throw new functions.https.HttpsError('invalid-argument', 'Invalid room name format');
        }
        // Get LiveKit credentials from environment variables
        const apiKey = (_a = functions.config().livekit) === null || _a === void 0 ? void 0 : _a.api_key;
        const apiSecret = (_b = functions.config().livekit) === null || _b === void 0 ? void 0 : _b.api_secret;
        if (!apiKey || !apiSecret) {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to generate LiveKit token');
    }
});
/**
 ...
 * @param context - Firebase function context with auth info
 */
exports.validateRoomAccess = functions.https.onCall(async (data, context) => {
    try {
        // Verify user is authenticated
        if (!context.auth) {
        ...
        if (!roomName.startsWith('privacycall_')) {
            throw new functions.https.HttpsError('permission-denied', 'Access denied to this room');
        }
        ...
        throw new functions.https.HttpsError('internal', 'Failed to validate room access');
    }
});
/**
 * Track Call Usage
...
exports.trackCallUsage = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!roomName || durationMinutes == null) {
            throw new functions.https.HttpsError('invalid-argument', 'roomName and durationMinutes are required');
        }
